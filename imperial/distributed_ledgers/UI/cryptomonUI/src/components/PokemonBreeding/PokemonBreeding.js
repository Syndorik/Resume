...
const BreeingButton = (props) => {
    const classes = useStyles();
    let mcolor = "primary"
    return (
        <Button variant="contained" color={mcolor} style={{ "width": "80%" }} onClick={() => { props.BreedCallBack(props.id) }}>
            {props.selectBreeder ? "Select First parent" : "Select Second parent"}
        </Button>
    );
}
                ...
                <CardFooter chart className={classes.centerAlign} >
                    ...
                    <GridContainer >
                        <GridItem xs={12} sm={12} md={12}>
                            <BreeingButton id={props.id} BreedCallBack={props.BreedCallBack} selectBreeder={props.selectBreeder} />
                        </GridItem>
                    </GridContainer>
                </CardFooter>
            </Card>
