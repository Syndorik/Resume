...
const BuyButton = (props) => {
    ...
    return (
        <Button variant="contained" color="primary" style={{ "width": "80%" }} onClick={() => {
            props.api.buyFromStore(props.crt, props.id, props.price).then(() => { window.location.reload() });
        }}>
            BUY
        </Button>
    );
                ...
                <CardFooter chart className={classes.centerAlign} >
                    ...
                    <GridContainer >
                        <GridItem xs={12} sm={12} md={12}>
                            <BuyButton id={props.id} price={props.price} api={api} crt={crt} />
                        </GridItem>
                    </GridContainer>
                </CardFooter>
            </Card>
